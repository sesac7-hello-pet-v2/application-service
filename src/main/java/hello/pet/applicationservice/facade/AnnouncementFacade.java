...
public class AnnouncementFacade {
    ...
    public AnnouncementCompletionResponse completeAnnouncement(Long announcementId, Long userId) {
        try {
            return announcementServiceClient.completeAnnouncement(announcementId, userId);
        } catch (FeignException.NotFound e) {
            throw new EntityNotFoundException("해당 번호의 공고를 찾을 수 없습니다. id=" + announcementId);
        }
    }

    public void cancelAnnouncementCompletion(Long announcementId, Long userId) {
        try {
            announcementServiceClient.cancelAnnouncementCompletion(announcementId, userId);
        } catch (FeignException.NotFound e) {
            throw new EntityNotFoundException("해당 번호의 공고를 찾을 수 없습니다. id=" + announcementId);
        }
    }
